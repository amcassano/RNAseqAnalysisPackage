...
export_genelist <- function(tab, filename, direction = NULL) {
  ...
  if (is.null(direction)) {utils::write.csv(tab, file = BiocGenerics::paste0(filename, ".csv"))}
  ...
  # if direction is "up", export only those genes with a + change metric
    ...
    utils::write.csv(upreg, file = BiocGenerics::paste0(filename, "_upregulated", ".csv"))
  }
  ...
  # if direction is "down", export only genes with - change metric
    ...
    utils::write.csv(downreg, file = BiocGenerics::paste0(filename, "_downregulated", ".csv"))
  }

  # if no direction is given or if it is anything other than up or down export all genes
  else {utils::write.csv(tab, file = BiocGenerics::paste0(filename, ".csv"))}
}
